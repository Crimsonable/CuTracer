...
template <typename Saver, size_t blocksize, typename DataType, int Dim,
          typename Exp, typename... Args>
__device__ void
WrapperKernel(ExpWrapper<Tensor<DataType, Dim, Device::GPU>> dst,
              const ExpWrapper<Exp> exp, Shape<2> dst_shape, size_t size,
              Args &&...args) {
  const int64_t tid = blockIdx.x * blocksize + threadIdx.x;
  if (tid < size) {
    const int64_t x = tid % dst_shape[0];
    const int64_t y = tid / dst_shape[0];
...
template <typename Saver, size_t blocksize, typename DataType, int Dim,
          typename Exp, typename... Args>
__global__ void
WrapperKernelCaller(ExpWrapper<Tensor<DataType, Dim, Device::GPU>> dst,
                    const ExpWrapper<Exp> exp, Shape<2> dst_shape, size_t size,
                    Args... args) {
...
template <typename Saver, typename DataType, int Dim, typename Exp,
          typename... Args>
inline void GPUExecute(ExpWrapper<Tensor<DataType, Dim, Device::GPU>> dst,
  ...
  if (num_blocks < CudaPars::MaxBlocksPerGrid) {
    ...
    WrapperKernelCaller<Saver, CudaPars::SugThreadsPerBlock>
        <<<griddim, blockdim>>>(dst, exp, dst_shape, dst_shape.size(), args...);
  }
}

template <typename Saver, typename DataType, int Dim, typename Exp,
          OperatorType exp_type>
struct GPUEngine<Saver, Tensor<DataType, Dim, Device::GPU>, Exp, exp_type> {
  template <typename... Args>
  inline static void dispatch(Tensor<DataType, Dim, Device::GPU> *dst,
                              const ExpBase<Exp, DataType, exp_type> &exp,
                              Args &&...args) {
    GPUExecute<Saver, DataType, Dim, Exp>(MakeExpWrapper(*dst), dst->shape,
