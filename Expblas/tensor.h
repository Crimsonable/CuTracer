...
public:
  ...
  BlasForceInline size_t stride() const { return this->derived_to().stride(); }
...
public:
  ...
  BlasForceInline size_t stride() const { return this->data->stride; }
...
template <typename T, Device device> auto Eye(size_t width) {
...
auto max(const ExpBase<Exp, T, exp_type> &exp, int axis) {
  return ReduceTemplate<Meta::traits<Exp>::device>::template eval<
      OP::SAVER::assign, OP::Reduce::max>(exp, axis);
}

template <typename T, typename Exp, OperatorType exp_type>
auto min(const ExpBase<Exp, T, exp_type> &exp, int axis) {
  return ReduceTemplate<Meta::traits<Exp>::device>::template eval<
