...
struct ExpWrapper<FTensor<T, shapes...>> {
  ...
  ExpWrapper(const FTensor<T, shapes...> &data) : data(data.dataptr()) {}
...
template <typename Saver, typename Container, typename T, typename Exp,
          OperatorType exp_type, typename... Args>
inline void nonPacketExecute(TensorBase<Container, T, Device::CPU> *dst,
